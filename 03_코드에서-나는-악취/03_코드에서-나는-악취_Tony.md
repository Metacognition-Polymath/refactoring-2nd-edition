...
- 하나의 모듈이 서로 다른 이유들로 인해 여러 방식으로 변경되는 일이 많을 때
- 단일 책임 원칙 (SRP; Single Responsibility Principle) 위배
- 설명에 산탄총 수술 부분도 섞여 있는 느낌

### 적용 가능한 기법

- 맥락을 명확하게 분리하는 것에 초점
- `6.11. 단계 쪼개기`
- 각 맥락에 해당하는 적절한 모듈을 만들어 `8.1. 함수 옮기기`
- 여러 맥락의 일에 관여하는 함수가 있다면 옮기기 전에 `6.1. 함수 추출하기`
- 모듈이 클래스인 경우 `7.5. 클래스 추출하기`
...
- "프로그램을 모듈화 할 때는 코드를 여러 영역으로 나누고 영역 안에서 이뤄지는 상호 작용은 최대한 늘리고 영역 사이에서 이뤄지는 상호 작용은 최소로 줄이는 데 주력한다."

### 적용 가능한 기법

- `8.1. 함수 옮기기`
- 함수의 일부분만 기능 편애가 있는 경우 `6.1. 함수 추출하기` 후 `8.1. 함수 옮기기`
- 어디로 옮길지 명확하게 드러나지 않는 경우
  - 함수를 쪼개거나
